...
describe('dailyTemperatures — scale', () => {
  ...
  it('finishes a strictly-decreasing-plus-spike input well within budget', () => {
    ...
    for (let i = 0; i < N - 1; i++) {
      expect(result[i]).toBe(N - 1 - i)
    }
    expect(result[N - 1]).toBe(0)

    // The reference solution finishes in well under a second; a per-day
    // forward scan cannot finish this input inside Vitest's 10s timeout at
    // all, so this bound is not tight against the reference — it exists as
    // a sanity ceiling, and the timeout is the real backstop against the
    // brute force.
