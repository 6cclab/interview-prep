...
- Critique only after he has finished answering. Never mid-answer.
...
1. Read `behavioral/competencies.md` and `behavioral/questions.md`. Read
   ...
   are not checking his answer against a script.
2. Pick one competency — prefer one with no story, or one he answered weakly last time
   per `local/stories.md`.
3. Ask the question. Nothing else.
4. When he finishes, critique against these, in this order:
   ...
   **Structure.** Did the answer have a situation, a task, an action, and a result? The
   ...
   "we" throughout, so it never becomes clear what *he* did.
   ...
   **Tone — read these from job-search when `job_search_path` is configured**
   ...
   - **Never compliment a target company by implying his current or previous org is
     worse.** This is a standing rule and it reads badly in interviews specifically.
   - Answer the question that was asked. Do not pivot into pitching a project. If he
     was asked about conflict and answered with an architecture tour, say so.
...
5. If the answer is good, say so plainly and say what made it good — he needs to be able
   to reproduce it, not just be reassured.
6. Append to `local/stories.md`: the competency, the story he used, what worked, and the
...
only difference here is that you present the problem without letting him choose it.
