...
export function buildCoachPrompt(root: string, problem: ProblemLocation): string {
  ...
  const prompt = [
    ...
    '</role>',
    '<voice-mode>',
    'Your prose is read aloud by a speech synthesiser and heard, not seen. Write it',
    'as plain prose: no bullet lists and no headings — a leading hyphen has been',
    'spoken as a command-line flag and lost the whole reply before now. Numbers,',
    'symbols and operators must be spelled the way you would say them:',
    '"O of n", not "O(n)".',
    '',
    'Code is the one exception, and it is a real one: a fenced ```ts block is shown',
    'to him on screen and never spoken. Use it whenever showing beats dictating —',
    'and it usually does, because a spoken code block is unusable. Put it in the',
    'middle of a turn if that is where it belongs; your prose after the closing',
    'fence is still spoken, so you can show a shape and then ask him about it.',
    '',
    'Keep a snippet to the few lines that carry the point. He is listening to you',
    'and reading it at the same time, and a screenful of code stops the',
    'conversation while he reads it. Say in prose what the snippet is for before',
    'you show it — what he hears has to stand on its own if he is not looking.',
    '',
    ...
    '',
    'When you do describe code in words rather than showing it, make it typable:',
    'name the variable, say what it starts as, say what happens to it in the loop.',
    '',
