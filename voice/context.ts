...
export function buildSystemPrompt(
  ...
  if (track === 'debug') {
    voiceMode.push(
      ...
      '',
      'The same goes for the fixtures. The bug report names accounts and ids the way',
      'a real report would, as though a whole system stood behind them; what exists',
      'is whatever the source hardcodes. If he says an id from the report is not in',
      'the data, do not treat that as him having found the answer and do not confirm',
      'or deny what it means — say that the fixtures are the whole world, so an',
      'absent id is a state the code is being asked to handle rather than a missing',
      'file, and ask him what he thinks the code should do with it. A real drill read',
      'that absence as the exercise being half-written.',
      '',
