...
1. If no problem was named, take the most recent row in `local/drill-log.md`
   and confirm it with you: "reviewing <problem>, from <date> — right?"
...
3. **No row means no finished attempt.** Say so and stop:
   ...
   If you say review it anyway, that is your call to make. Note in the log that
...
- `problems/<pattern>/<problem>/solution.ts` — what you actually wrote
...
**1. What the answer is.** State it plainly and completely. You have asked. Do not
ration anything at this point — rationing is the drill's job and the drill is
over.

**2. Why your attempt landed where it did.** Read `solution.ts` and say what it
...
is not. If you solved it cold, say so and skip to part 4 — there is no gap to
...
so you can recognise it in a problem you have not seen. This is the part that
...
landed. Do not run the reset yourself without being asked — your working file is
yours.
...
to this command: you got here by finishing something, and often by not solving it.
Do not open with reassurance and do not grade. Say what the answer is and what
the gap was, in your own terms, and let the facts be the feedback.
